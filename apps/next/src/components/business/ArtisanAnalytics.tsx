...
export function ArtisanAnalytics() {
  ...
  const fetchStats = async () => {
    ...
    if (productIds.length > 0) {
      ...
      if (orderItems) {
        const uniqueOrders = new Set(orderItems.map((i: { order_id: any; }) => i.order_id));
        orderCount = uniqueOrders.size;
        revenue = orderItems.reduce((s: number, i: { unit_price: number; quantity: number; }) => s + i.unit_price * i.quantity, 0);
      }
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        {cards.map((card, i) => (
          <motion.div key={card.label} className="border-2 border-foreground p-6">
