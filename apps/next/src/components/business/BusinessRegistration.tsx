    ...
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="space-y-2">
          ...
          <Input value={form.business_name} onChange={(e: { target: { value: any; }; }) => setForm({ ...form, business_name: e.target.value })} className="border-2 border-foreground font-body" placeholder="Your business name" />
        </div>
        <div className="space-y-2">
          <Label className="font-display text-xs tracking-widest">BUSINESS TYPE</Label>
          <Select value={form.business_type} onValueChange={(v: any) => setForm({ ...form, business_type: v })}>
        ...
        <div className="space-y-2">
          ...
          <Input value={form.tax_id} onChange={(e: { target: { value: any; }; }) => setForm({ ...form, tax_id: e.target.value })} className="border-2 border-foreground font-body" placeholder="e.g., 1000123456" />
        </div>
        <div className="space-y-2">
          <Label className="font-display text-xs tracking-widest">REGISTRATION NUMBER</Label>
          <Input value={form.registration_number} onChange={(e: { target: { value: any; }; }) => setForm({ ...form, registration_number: e.target.value })} className="border-2 border-foreground font-body" placeholder="Business registration number" />
        </div>
        <div className="space-y-2">
          <Label className="font-display text-xs tracking-widest">REGISTRATION STATUS</Label>
          <Select value={form.registration_status} onValueChange={(v: any) => setForm({ ...form, registration_status: v })}>
            <SelectTrigger className="border-2 border-foreground font-body"><SelectValue /></SelectTrigger>
            <SelectContent>
        ...
        <div className="space-y-2">
          ...
          <Input value={form.business_email} onChange={(e: { target: { value: any; }; }) => setForm({ ...form, business_email: e.target.value })} className="border-2 border-foreground font-body" placeholder="business@example.com" />
        </div>
        <div className="space-y-2">
          <Label className="font-display text-xs tracking-widest">BUSINESS PHONE</Label>
          <Input value={form.business_phone} onChange={(e: { target: { value: any; }; }) => setForm({ ...form, business_phone: e.target.value })} className="border-2 border-foreground font-body" placeholder="+256 700 000000" />
        </div>
        <div className="space-y-2">
          <Label className="font-display text-xs tracking-widest">CITY</Label>
          <Input value={form.business_city} onChange={(e: { target: { value: any; }; }) => setForm({ ...form, business_city: e.target.value })} className="border-2 border-foreground font-body" placeholder="e.g., Kampala" />
        </div>
        <div className="md:col-span-2 space-y-2">
          <Label className="font-display text-xs tracking-widest">BUSINESS ADDRESS</Label>
          <Input value={form.business_address} onChange={(e: { target: { value: any; }; }) => setForm({ ...form, business_address: e.target.value })} className="border-2 border-foreground font-body" placeholder="Full business address" />
        </div>
        <div className="md:col-span-2 space-y-2">
          <Label className="font-display text-xs tracking-widest">BUSINESS DESCRIPTION</Label>
          <Textarea value={form.description} onChange={(e: { target: { value: any; }; }) => setForm({ ...form, description: e.target.value })} className="border-2 border-foreground font-body min-h-[100px]" placeholder="Describe your business, products, and story..." />
        </div>
      </div>
